									...
									const WedgeData<int>& index, Mesh& mesh)
{
	...
	int cones = initializeConeSet(K, index, isCone, mesh);
	if (mesh.boundaries.size() == 0) S -= cones;
	if (gProgress && !gProgress(0)) return 0;

	// compute scale factors
	DenseMatrix u;
	if (!computeScaleFactors(u, K, A, isCone, index, mesh)) return false;
	if (gProgress && !gProgress(float(1) / (S + 1))) return 0;
	...
	for (int i = 0; i < S; i++) {
		...
		// compute scale factors
		if (!computeScaleFactors(u, K, A, isCone, index, mesh)) return false;
		if (gProgress && !gProgress(float(i + 1) / (S + 1))) return 0;
	}
											...
											std::shared_ptr<BFFData> data, Mesh& mesh)
{
	...
	if (gProgress && !gProgress(0)) return ConePlacement::ErrorCode::factorizationFailed;
